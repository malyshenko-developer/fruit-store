...
export async function getProducts(params: GetProductsParams = {}): Promise<PaginatedProducts> {
  ...
  if (params.minScreenSize !== undefined)
    query.set("min_screen_size", String(params.minScreenSize));
  if (params.maxScreenSize !== undefined)
    query.set("max_screen_size", String(params.maxScreenSize));
...
export async function searchProducts(query: string, page: number = 1): Promise<PaginatedProducts> {
